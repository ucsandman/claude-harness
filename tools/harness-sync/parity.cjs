...
function inspectHarness(h) {
  ...
  const rules = {
    ...
    generated: !!(rulesRaw && (rulesRaw.includes('GENERATED by agnostic-ai') || rulesRaw.includes('harness-sync/sync.cjs'))),
  };
...
try {
  execFileSync('node', ['C:/Projects/agnostic-ai/engine/harness/cli.cjs', 'port', '--check'], {
    stdio: 'ignore',
  });
  syncState = 'in sync';
} catch {
...
<div class="wrap">
  ...
  <div class="cmds">
    ...
    ${data
      ...
      .join('\n    ')}
    <div style="margin-top:10px">Regenerate: <code>npm run port</code> in C:Projectsgnostic-ai (tools/harness-sync/sync.cjs is retired)</div>
    <div>Verify only: <code>npm run port:check</code></div>
